...
def delete_messages(user_id, phone_number):
    ...
    try:
        ...
        return [ChatMessage(**message.to_dict()).to_dict() for message in messages]
...
def get_messages(user_id, phone_number):
    """Obtiene la conversación de un usuario con un contacto específico."""
    try:
        contact_ref = get_contact_ref(user_id, phone_number)
        if not contact_ref:
        ...
        return [message.to_dict() for message in mensajes_ref.get()]
