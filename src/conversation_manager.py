...
from src.utils.open_ai_tools import (
...
class ConversationManager:
    ...
    def manage_incoming_message(self, message: Dict[str, Any]):
        ...
        try:
            ...
            user_data = get_contact_data(self.whatsapp_api_handler.from_whatsapp_id, number)

            # Procesar mensaje y generar respuesta
            response = self._process_message_by_type(
                message_type, text, message, user_data
            )
    ...
    def _handle_tool_calls(self, response, number):
        ...
        for tool_call in response.tool_calls:
            ...
            if function_name == "notify_payment_mail":
                ...
                function_response = "Email sent"

            elif function_name == "store_user_data":
                store_user_data(
                    self.whatsapp_api_handler.from_whatsapp_id, number, args
                )
                function_response = "User data stored"
    ...
    def _register_message(
        self, number: str, text: str, message_id: Optional[str]
    ) -> None:
    ...
    def _handle_text_message(self, text: str, number: str, user_data: Dict[str, Any]):
        ...
        return self._generate_response_from_sections(
            relevant_sections, user_data, messages
        )
    ...
    def _generate_response_from_sections(
        ...
        try:
            ...
            system_prompt = (
                ...
                f"\nTen en cuenta los datos del usuario: {user_data}"
            )
            ...
            try:
                ...
                response = client.chat.completions.create(
                    ...
                    messages=[{"role": "system", "content": system_prompt}, *messages],
                    tools=[
                        get_store_user_data_tool(user_data),
                        get_notify_payment_mail_tool(),
                    ],
                    max_tokens=150,
                    temperature=0.1,
