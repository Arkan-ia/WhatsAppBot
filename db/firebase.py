...
def create_project(nombre_proyecto):
    ...
    try:
        ...
        proyecto_ref.set(
            {"nombre": nombre_proyecto, "timestamp_creacion": firestore.SERVER_TIMESTAMP}
        )
...
def add_contact(ws_id, phone_number, display_name):
    ...
    try:
        ...
        if not user_ref:
            print(f"No se encontró ningún usuario al añadir contacto {phone_number} para {ws_id}")
            return
...
def get_contact_ref(ws_id, phone_number):
    ...
    try:
        user_ref = db.collection("users").where("ws_id", "==", ws_id).limit(1).get()
        if not user_ref:
            print(f"No se encontró ningún usuario al obtener contacto {phone_number} para {ws_id}")
            return None
        user_doc = user_ref[0].reference
        contact_query = (
...
def add_message(ws_id, phone_number, message, is_sender):
    ...
    try:
        ...
        user_ref = db.collection("users").where("ws_id", "==", ws_id).limit(1).get()
        if not user_ref:
            print(f"No se encontró ningún usuario al añadir mensaje para {phone_number} para {ws_id}")
            return
        user_doc = user_ref[0].reference
...
def get_conversation(ws_id, phone_number):
    ...
    try:
        ...
        user_ref = db.collection("users").where("ws_id", "==", ws_id).limit(1).get()
        if not user_ref:
            print(f"No se encontró ningún usuario al obtener conversación para {phone_number} para {ws_id}")
            return None
        user_doc = user_ref[0].reference
...
def upload_media_to_storage(image, path):
    ...
    try:
        ...
        blob.upload_from_string(image.content, content_type=image.headers["content-type"])
...
def get_whatsapp_token(ws_id: str) -> str:
    try:
        user_ref = db.collection("users").where("ws_id", "==", ws_id).limit(1).get()
        if not user_ref:
            print(f"No se encontró ningún usuario al obtener token de WhatsApp para {ws_id}")
            return
